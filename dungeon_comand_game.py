import random

print('================================================')
print('\t\t#Dungeon')
print('================================================\n\n')

enemies = ['Skeleton', 'Assassin', 'Vampire', 'Mummy']

player_health = 100
number_of_potions = 4
health_potion = 30
enemy_drop_chance = 50
enemy_kill_count = 0
player_input1 = ''

while True:
    enemy = random.choices(enemies)
    enemy_health = random.randint(10, 76)
    print('================================================')
    print(f'#       {enemy} has appeared        #')
    print(f'Your HP: {player_health}')
    print(f"{enemy}'s HP: {enemy_health}        #")
    print('================================================')

    while enemy_health > 1:
        print('================================================')
        print('What do you want to do?')
        print('1 Attack')
        print('2 Drink Health Potion')
        print('3 Run')
        player_input = input('Enter your move: ')
        if player_input == '1':
            enemy_damage = random.randint(10, 40)
            player_damage = random.randint(0, 10)
            
            enemy_health -= enemy_damage
            player_health -= player_damage
            
            print(f'You  caused the {enemy} {enemy_damage} damage in the attack')
            print(f'In retaliation! you caused {player_damage} damage')
            print(f'Your HP :{player_health}')
            print(f"{enemy_health}'s HP: {enemy_health}")
            if player_health < 1:
                print('You are too weak to battle now!\n You are brought out of game.')
                break
        elif player_input == '2':
            if number_of_potions > 0:
                print('You drank a potion.')
                player_health += health_potion
                number_of_potions -= 1
                print(f'Your HP: {player_health}')
                print(f'You now have {number_of_potions} left.')
            else:
                print('You do not have a single potion')
                print('You must kill an anemy in order to get a potion')
        elif player_input == '3':
            print(f'You run away from the {enemy}')
            break
        else:
            print('Invalid comand! enter 1, 2 or 3.')
        
    if player_health < 0:
        print('Thank you for playng DUNGEON!\n You did the best.')
        break
    if enemy_health < 0:
        print(f'You defeated the {enemy}')
        print('================================================')
        enemy_kill_count += 1
        if random.randint(0, 100) > enemy_drop_chance:
            print(f'{enemy} dropped a potion.')
            number_of_potions += 1
            print(f'You now have {number_of_potions} with you')
            print('================================================')
